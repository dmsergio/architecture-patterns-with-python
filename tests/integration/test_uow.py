...
def try_to_allocate(orderid: AnyStr, sku: AnyStr, exceptions: List):
...
def test_uow_can_retrieve_a_batch_and_allocate_to_it(sqlite_session_factory):
    ...
    with uow:
        product = uow.products.get(sku="sku-001")
        line = model.Orderline("order-01", "sku-001", 40)
        product.allocate(line)
        uow.commit()

    batchref = get_allocated_batch_ref(session, "order-01", "sku-001")
...
def test_concurrent_updates_to_version_are_not_allowed(postgres_session_factory):
    ...
    order1, order2 = random_element("order"), random_element("order")
    ...
    orders = list(session.execute(
        "SELECT order_line_id FROM allocations "
